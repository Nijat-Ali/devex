...
function NotifBell({ userId }: any) {
  ...
  return (
    <div style={{ position: "relative" }}>
      ...
      {open && <div style={{ position: "absolute", right: 0, top: "calc(100% + 8px)", width: 300, ...card0(), zIndex: 300, overflow: "hidden" }}>
    ...
    <div style={{ background: C.bg, minHeight: "100vh", color: C.tp }}>
      <nav style={{ borderBottom: `1px solid ${C.bd}`, padding: "0 32px", display: "flex", alignItems: "center", justifyContent: "space-between", height: 60, position: "sticky", top: 0, background: C.bg + "ee", backdropFilter: "blur(12px)", zIndex: 100 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
          ...
          <div style={{ display: "flex", gap: 2 }}>{[["marketplace", "Marketplace"], ["experts", "Experts"], ["learners", "Talent"]].map(([p, l]) => <button key={p} onClick={() => go(p)} style={{ padding: "6px 12px", borderRadius: 6, border: "none", background: page === p ? C.accBg : "transparent", color: page === p ? C.acc : C.ts, fontSize: 13, fontWeight: page === p ? 600 : 400, cursor: "pointer" }}>{l}</button>)}</div>
        </div>
...
function LearnerDash({ user, go, data }: any) {
  ...
  return <div>
    ...
    {tab === "overview" && <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      {purchases.length === 0 && <Empty icon="📋" title="No assignments yet" body="Browse the marketplace and buy your first assignment." action={<Btn onClick={() => go("marketplace")}>Browse marketplace →</Btn>} />}
    </div>}
...
function ExpertDash({ user, go, data, refresh, toast }: any) {
  ...
  return <div>
    ...
    {tab === "overview" && <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      {myA.length === 0 && <Empty icon="📋" title="No assignments yet" action={<Btn onClick={() => go("create-assignment")}>Create your first assignment →</Btn>} />}
...
function Dashboard({ user, setUser, go, data, loading, refresh, toast }: any) {
  ...
  return <div style={{ background: C.bg, minHeight: "100vh", display: "grid", gridTemplateColumns: "200px 1fr", color: C.tp }}>
    <div style={{ background: "#060b16", borderRight: `1px solid ${C.bd}`, padding: "18px 12px", display: "flex", flexDirection: "column", minHeight: "100vh", boxSizing: "border-box", position: "sticky", top: 0, height: "100vh" }}>
    ...
    <div style={{ padding: "32px 36px", overflowY: "auto", maxHeight: "100vh", boxSizing: "border-box" }}>
